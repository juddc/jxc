...
public:
    ...
    TokenView(Token& start, size_t num_tokens, std::string_view source_view = {}) : start(&start), num_tokens(num_tokens), src(FlexString::make_view(source_view)) {}
    ...
    // Returns a string view representing this token span's source string.
    inline std::string_view get_source_view() const { return src.as_view(); }
};
...
public:
    ...
    // Returns a string view representing this token list's source string.
    std::string_view get_source_view() const;
