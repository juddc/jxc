...
Serializer& Serializer::value_bytes(const uint8_t* data, size_t data_len, StringQuoteMode quote)
{
    return settings.encode_bytes_as_hexbytes
        ? value_bytes_hex(data, data_len, quote)
        : value_bytes_base64(data, data_len, quote);
}
...
Serializer& Serializer::value_bytes_hex(const uint8_t* data, size_t data_len, StringQuoteMode quote)
{
    ...
    detail::MiniBuffer<char, 3> hexbuf;
    std::string_view hexbuf_view = hexbuf.as_view(hexbuf.capacity() - 1);

    if (settings.pretty_print && settings.get_target_line_length() > 0 && (int64_t)hexbytes::get_hexbytes_string_size(data_len) > get_num_cols_remaining_on_line())
    {
        // multi-line version
        last_token_size += output.write('(');

        int64_t num_bytes_per_line = ((int64_t)settings.target_line_length - get_current_indent_width()) / 2;
        if (num_bytes_per_line < 10)
        {
            num_bytes_per_line = 10;
        }
        ...
        while (bytes_remaining > 0)
        {
            ...
            const size_t line_num_bytes = (bytes_remaining > (size_t)num_bytes_per_line) ? (size_t)num_bytes_per_line : bytes_remaining;
            for (size_t i = 0; i < line_num_bytes; i++)
            {
                hexbytes::detail::byte_to_hex(data[data_offset + i], hexbuf[0], hexbuf[1]);
                last_token_size += output.write(hexbuf_view);
            }
            bytes_remaining -= line_num_bytes;
            data_offset += line_num_bytes;
        }
    ...
    else
    {
        ...
        {
            hexbytes::detail::byte_to_hex(data[i], hexbuf[0], hexbuf[1]);
            last_token_size += output.write(hexbuf_view);
        }
    }
